...
async function cekKey(req, res, next) {
	...
	if (!apikey ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento apikey"})  
...
router.get('/api/dowloader/fbdown', cekKey, async (req, res, next) => {
	...
	if (!url ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento url"})  
...
router.get('/api/textpro/pencil', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento text"})   
...
router.get('/api/textpro/glitch2', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento text"})   
	if (!text2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento text2"}) 
...
router.get('/api/maker/circle', cekKey, async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento url"})
...
router.get('/api/maker/darkness', cekKey, async (req, res) => {
	...
	var no = req.query.no
	if (!text ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento url"})
	if (!no ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento no"})
...
router.get('/api/maker/resize', cekKey, async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento url"})
	if (!width ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento width"})
	if (!height ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento height"})
...
router.get('/api/maker/attp', cekKey, async (req, res) => {
	...
	if (!text ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento text"})
...
router.get('/api/maker/emojimix', cekKey, async (req, res, next) => {
	...
	if (!emoji1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento emoji1"})
	if (!emoji2 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento emoji2"})  
...
router.get('/api/maker/welcome1', cekKey, async (req, res, next) => {
    ...
    if (!name ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento name"})  
	if (!grup ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento gpname"})  
    if (!member ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento member"})  
	if (!pp ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento pp"})  
    if (!bg ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento bg"})  
...
router.get('/api/maker/goodbye1', cekKey, async (req, res, next) => {
	...
	if ( !imgpp ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento pp Link pp dengan betul"}) 
	if ( !bgimg ) return res.json({ status : false, creator : 'Alip', message : "[!] insira o paramento pp Link bg dengan betul"}) 
...
router.get('/api/linkshort/tinyurl', cekKey, async (req, res, next) => {
	...
	if (!link ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento link"})  

    var islink = isUrl(link)
	if (!islink ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento url sahaja"})  
...
router.get('/api/linkshort/tinyurlwithalias', cekKey, async (req, res, next) => {
	...
	if (!link ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento link"})  
	if (!alias ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento alias"})  
...
router.get('/api/info/githubstalk', cekKey, async (req, res, next) => {
	...
	if (!user ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento user"})  
...
router.get('/api/info/translate', cekKey, async (req, res, next) => {
	...
	if (!text ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento text"})  
	if (!lang ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento lang.  boleh lihat list bahasa di https://cloud.google.com/translate/docs/languages"})  
	...
	translate(text, {to: lang}).then(data => {
		...
		res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento lang Dengan Betul.  boleh lihat list bahasa di https://cloud.google.com/translate/docs/languages"})
	})
...
router.get('/api/info/emoji', cekKey, async (req, res, next) => {
	...
	if (!emoji1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento emoji"})
      var hasil = emoji.get(emoji1)
       if (hasil == null) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento emoji dengan 1 emoji sahaja"})
           limitapikey(req.query.apikey)
           res.json({
			status: true,
	        creator: `${creator}`,
...
router.get('/api/islamic/surah', cekKey, async (req, res, next) => {
	...
	if (!text1 ) return res.json({ status : false, creator : `${creator}`, message : "[!] insira o paramento no"})  
